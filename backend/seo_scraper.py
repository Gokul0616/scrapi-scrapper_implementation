...
class SEOMetadataScraper:
    ...
    def __init__(self):
        self.name = "SEO Metadata Scraper"
        self.description = "Extract comprehensive SEO metadata including meta tags, Open Graph, Twitter Cards, JSON-LD structured data, headings, and technical SEO elements"
    
    async def scrape(self, page: Page, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        Args:
            ...
            params: {
                ...
                'extract_headings': bool - Extract H1-H6 tags (default: True)
                'extract_images': bool - Extract image metadata (default: True)
                'extract_links': bool - Extract internal/external links (default: False)
                'max_results': int - Maximum number of URLs to crawl (default: 1)
            }
        
        Returns:
            List of dictionaries containing SEO metadata
        """
        url = params.get('url')
        extract_headings = params.get('extract_headings', True)
        ...
        try:
            logger.info(f"🔍 Starting SEO metadata extraction for: {url}")
            ...
            logger.info(f"✅ Successfully extracted SEO metadata from: {url}")
    ...
    async def _extract_metadata(self, page: Page, url: str, status_code: Optional[int]) -> Dict[str, Any]:
        ...
        metadata['robots_txt'] = self._get_robots_url(url)
        metadata['sitemap_xml'] = self._get_sitemap_url(url)
