...
class EnhancedGlobalChatService:
    ...
    def __init__(self, db, user_id: str, conversation_id: Optional[str] = None):
        self.db = db
...
**🧊 DOMAIN TERMINOLOGY OVERRIDE (CRITICAL)**
- **ACTORS** = SCRAPERS / WEB SCRAPING TOOLS (e.g., Google Maps Scraper, Amazon Scraper).
- **LEADS / DATASETS** = RESULTS generated by a scraper.
- **RUNS / JOBS** = Individual EXECUTIONS of a scraper.
- **NEVER** assume "actors" refers to movies, theater, or the Sakila database. 
- **NEVER** suggest movie cast lists or SQL scripts when the user asks for actors. 
- **ALWAYS** assume the user is talking about the Scrapi platform.
...
- NEVER generate Python scripts, Node.js code, or scraper code using Playwright, Puppeteer, BeautifulSoup, requests, etc.
- You are already connected to the powerful Scrapi internal scraping engine!
- When user says "run a scraper", "build a scraper", "scrape X", or "make a scraper", they are commanding YOU to TRIGGER the internal scraper natively. 
- NEVER write code to do it. ALWAYS and ONLY output the FUNCTION_CALL to trigger the internal scraper!
...
**STRICT OUTPUT RULES:**
- NEVER include internal reasoning, logic checks, or decision marks in your response.
- DO NOT use markers like ❌ or ✅ in your output to the user.
- DO NOT say things like "(No navigation needed)" or "I've checked your request against my rules".
- Only respond with the actual conversational message.

**CRITICAL: DO NOT Navigate on Greetings or Casual Conversation**
- Simple greetings like "hello", "hi", "hey", "good morning" → Respond warmly, NO navigation.
- Casual conversation like "thanks", "ok", "cool", "awesome" → Respond naturally, NO navigation.
...
"""
    ...
    async def save_message(self, role: str, content: str, function_call: Optional[Dict] = None):
        ...
        try:
            ...
            # Update the conversation's updated_at if conversation_id exists
            if self.conversation_id:
                ...
                # Broadast chat update for real-time UI refresh (especially for assistant replies)
                if role == "assistant":
                    try:
                        from routes.notification_routes import broadcast_chat_update
                        fe_message = {
                            "role": role,
                            "content": content,
                            "timestamp": now
                        }
                        await broadcast_chat_update(self.user_id, self.conversation_id, fe_message)
                    except Exception as b_err:
                        logger.warning(f"Failed to broadcast chat update: {str(b_err)}")
        except Exception as e:
    ...
    async def clear_history(self):
        ...
        try:
            if self.conversation_id:
                await self.db.global_chat_history.delete_many({"user_id": self.user_id, "conversation_id": self.conversation_id})
                await self.db.chat_conversations.delete_one({"id": self.conversation_id, "user_id": self.user_id})
            else:
                await self.db.global_chat_history.delete_many({"user_id": self.user_id})
                await self.db.chat_conversations.delete_many({"user_id": self.user_id})
            return {"success": True}
        except Exception as e:
    ...
    async def chat(self, message: str) -> Dict[str, Any]:
        ...
        """
        try:
            # Ensure conversation exists (Supports client-provided IDs for instant sync)
            if not self.conversation_id:
                self.conversation_id = str(__import__('uuid').uuid4())
            
            # Use find_one to check if this conversation already exists for the user
            existing_conv = await self.db.chat_conversations.find_one({
            ...
            if not existing_conv:
                ...
                await self.db.chat_conversations.insert_one({
                    "id": self.conversation_id,
                    "user_id": self.user_id,
                    "title": title,
                    "created_at": datetime.now(timezone.utc).isoformat(),
...
DO NOT say "Starting..." or "I'll help you..." - ONLY output the FUNCTION_CALL.

**Requirement**: Respond naturally to greetings. Your internal reasoning process (e.g. checking if a command requires navigation) should NEVER be visible to the user.
...
{conversation_context}"""
            ...
            if self.provider == "openrouter":
                fallbacks.extend([
                    {"provider": "openrouter", "model": self.model, "api_key": self.api_key},
                ...
                fallbacks.append({"provider": self.provider, "model": self.model, "api_key": self.api_key})
            ...
            for fback in fallbacks:
                try:
                    ...
                    last_error = e
                    logger.warning(f"Model {fback['model']} failed (likely rate limit): {str(e)}")
                    continue  # Try next model
                    
            if response is None:
                logger.error(f"All fallback models failed. Last error: {str(last_error)}")
                response = "I'm currently experiencing high traffic rate limits across all free models. Please wait a minute and try again!"
            ...
            if function_calls:
                ...
                if fast_response:
                    ...
                    # Generate follow-up response with fallback logic
                    final_response = None
                    ...
                    for fback in fallbacks:
                        try:
                            ...
                            last_follow_up_error = e
                            continue
                            
                    if final_response is None:
                        logger.error(f"All fallback models failed for follow-up. Last error: {str(last_follow_up_error)}")
                        final_response = "I've completed the action, but I'm currently hitting a rate limit to summarize it fully."
