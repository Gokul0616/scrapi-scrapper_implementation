...
from services import get_proxy_manager, get_task_manager, LeadChatService, EnhancedGlobalChatService, get_email_validator
...
async def register(user_data: UserCreate):
    ...
    if owner_exists:
        ...
        # First user - can be owner
        user_role = "owner"
    else:
        user_role = "admin"  # Temporary default, will be updated via role selection
...
async def check_email(email: str):
    ...
    # Validate email (block temporary/disposable emails)
    email_validator = get_email_validator()
    is_valid, error_msg = email_validator.validate(email)
    if not is_valid:
        raise HTTPException(status_code=400, detail=error_msg)
...
async def suspend_user(
    ...
    await log_admin_action(
        ...
        user_id, 
        user['username'], 
        details=reason.get('reason') if reason else "Suspended by admin",
...
async def update_user_admin(
    ...
    if user.get('role') == 'owner' and current_user['id'] != user['id']:
         raise HTTPException(status_code=403, detail="Cannot modify owner account")

    allowed_updates = ['plan', 'role', 'organization_name']
    ...
    # Log details
    details = ", ".join([f"{k} changed to {v}" for k, v in update_data.items()])
