...
import { convertToOrganization } from '../services/organizationService';
import { Building2, AlertCircle, HelpCircle, User, ArrowRight } from 'lucide-react';
import GlobalModal from './GlobalModal';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from './ui/tooltip';

const ConvertToOrganizationModal = () => {
    const { theme } = useTheme();
    const { closeModal, modalData, isModalOpen } = useModal();
    const isOpen = isModalOpen('convert-to-organization');
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await convertToOrganization(formData);

            onSuccess && onSuccess(response);
            closeModal();
        } catch (error) {
            setApiError(error.response?.data?.detail || 'Failed to convert to organization');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <GlobalModal
            modalId="convert-to-organization"
        ...
        >
            <form onSubmit={handleSubmit} className="p-6">
                <div className={`mb-6 p-4 rounded-lg flex items-start space-x-3 ${theme === 'dark' ? 'bg-blue-900/20 border border-blue-800' : 'bg-blue-50 border border-blue-200'
                    }`}>
                    <AlertCircle className="w-5 h-5 text-blue-500 flex-shrink-0 mt-0.5" />
                    <div className="text-sm">
                        <h4 className={`font-semibold mb-1 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-800'}`}>
                            What happens when you convert?
                        </h4>
                        <ul className={`list-disc list-inside space-y-1 ${theme === 'dark' ? 'text-blue-300' : 'text-blue-700'}`}>
                            <li>Your personal account becomes an organization</li>
                            <li>You become the owner of the new organization</li>
                            <li>All your actors, runs, and datasets will move to the organization</li>
                            <li>You can invite other members to collaborate</li>
                        </ul>
                    </div>
                </div>
                ...
                <div className="flex justify-end space-x-3 mt-8">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="px-4 py-2 rounded-md text-sm font-medium bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm flex items-center gap-2"
                    >
                        {loading ? 'Converting...' : (
                            <>
                                Convert Account
...
export default ConvertToOrganizationModal;
