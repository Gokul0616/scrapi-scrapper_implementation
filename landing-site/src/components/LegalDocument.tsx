...
const LegalDocument: React.FC<LegalDocumentProps> = ({ onOpenCookieSettings }) => {
    const { docId } = useParams<{ docId: string }>();
    const navigate = useNavigate();
    const [activeSection, setActiveSection] = useState<string | null>(null);
    const [data, setData] = useState<DocumentData | null>(null);
    const [loading, setLoading] = useState(true);
    const [sidebarLinks, setSidebarLinks] = useState<SidebarGroup[]>([]);
        ...
        const fetchSidebarData = async () => {
            ...
            try {
                ...
                if (categoriesResponse.ok && policiesResponse.ok) {
                    ...
                    const categoryMap = new Map<string, SidebarGroup>();
                    ...
                    policiesData.documents.forEach((doc: any) => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // Need to handle this better maybe with a component
                    // navigate('/404'); 
        ...
        const handleScroll = () => {
            ...
            sections.forEach((section) => {
                ...
                if (scrollPosition >= sectionTop && scrollPosition < sectionTop + sectionHeight) {
                    currentActiveId = HTMLElement.getAttribute('id');
                }
            });
    ...
    if (!loading && !data && !sidebarLoading) {
        return <NotFound onOpenCookieSettings={onOpenCookieSettings || (() => { })} />;
    }
